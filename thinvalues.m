...
% parameter values from "thin values" in article doi:10.1016/j.pocean.2007.09.002
...
param.HN = 0.212; %[mmol N/m³]

param.kp = 0.045; %[m² (mmol N)^-1] self shading
param.kw = 0.045; %[m⁻1] &background turbidity
param.Av = 2.3*10^(-5)*(60*60*24); %[m^2 day^-1] 
param.loss = 0.046; %[day⁻1] 
param.gamma = 0.648; %[m^3 (mmol N)^-1 day^-1] grazing mortality 
param.zt = 20; %meters % thermocline depth in stratified waters 
param.I0 = 600*2; %[umol photons m⁻2 s⁻1] 
param.pmax = 0.5; %[day⁻1] 
param.NB = 22.01; %nutrient concentration at bottom [mmol N m^-3]
param.tau = 0.115; % [day^-1]
param.w = 8.18; % [m day^-1] sinking velocity
...
% shading flat
% colorbar
% title('C. Phytoplankton growth over time [mmol Nitrogen m⁻3]','FontSize', 20)
...
% shading flat
% colorbar
% title('C. Nutrient concentration over time [mmol Nitrogen m⁻3]','FontSize', 20)
% xlabel('Time [days]','FontSize', 16)
% ylabel('Depth [meters]','FontSize', 16)
% 
% figure; 
...
% shading flat
% colorbar
% title('C. Detritus concentration over time [mmol Nitrogen m⁻3]','FontSize', 20)
% xlabel('Time [days]','FontSize', 16)
% ylabel('Depth [meters]','FontSize', 16)
...
title('C. Steady state solution for the state variables, time=3000 days','FontSize',12)
